...
| Role | Core Features |
|------|--------------|
|  **Citizen** | Submit pickup requests with waste photo & GPS location · Track status in real-time · View pickup history · Cancel pending requests · Dashboard with personal metrics |
...
|  **Admin** | Platform-wide analytics · User management · Dealer verification (approve/reject) · Create inventory lots from completed pickups · Manage material categories & pricing rules · Archive/restore lots |
...
-  Alembic database migrations with PostgreSQL (SQLite for local dev)
-  Docker Compose for one-command local development
-  GitHub Actions CI for backend (lint + type-check + test) and frontend (lint + test + build)
